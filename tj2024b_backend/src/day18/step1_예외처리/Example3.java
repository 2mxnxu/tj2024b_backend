...
public class Example3 {
	...
	public static void method() throws NullPointerException { 
		...
		// 예측 안되면 => 코드 실행 => 예외확인 => 예외처리 
		// static : 프로그램이 실행횔때 static 키워드가 존재하는 변수와 메소드는 메모리에 우선 할당되고 프로그램이 종료되면 사라진다.
		// --> 정적 : 메모리 고정, static vs 동적 : 인스턴스, new(생성), GC(자동삭제 -> 참조를 당하지 않으면)
		// --> 인스턴스 없어도 이미 메모리(실체)가 존재하므로 바로 사용이 가능, static 없는 변수 나 메소드는 인스턴스 (메모리/실체)가 필요.
	}
	
	public static void main(String[] args) {
		// 1. 메소드 호출, 예외처리 발생 : 호출한 메소드가 throws 이용하여 메소드 내부 예외를 반환했기 때문에
		try {
			loadClass ("a.text", "java.lang.String");
		}
